            ...
            >
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-2 rounded-md focus:outline-none " required />
                    </div>
                    <div>
                        <label htmlFor="email" className="block mb-2 text-secondary-200">Email</label>
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-2 rounded-md focus:outline-none " required />
                    </div>
                    <div>
                        <label htmlFor="message" className="block mb-2 text-secondary-200">Message</label>
                        <textarea value={message} onChange={(e) => setMessage(e.target.value)} className="w-full px-4 py-2 rounded-md focus:outline-none " rows="5" required></textarea>
                    </div>
                    <div>
                        <button type="submit" className="px-4 py-2 font-bold text-base border-2 border-secondary-300 text-white hover:text-secondary-200 rounded-lg">Submit</button>
                    </div>
                </form>
            </motion.div>
            <SocialMediaLinks />
